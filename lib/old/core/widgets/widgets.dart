...
class ColorPicker extends StatefulWidget {
  ...
  _ColorPickerState createState() => _ColorPickerState(
      currentColor, initColorIndex, onChanged, onDone, colorList, child);
}
...
class _ColorPickerState extends State<ColorPicker> {
  ...
  Function(Color color, int index) onChanged;
  Function(Color color, int index) onDone;
  _ColorPickerState(this.currentColor, this.initColorIndex, this.onChanged,
      this.onDone, this.colorList, this.child) {
    ...
    child ??= AnimatedContainer(
      ...
      constraints: BoxConstraints(
          minWidth: 48, minHeight: 48, maxHeight: 48, maxWidth: 48),
              ...
              ButtonBar(
                ...
                children: [
                  FlatButton(
                    child: Text(
                      'Cancel',
                      style: TextStyle(color: appliedTheme.accentColor),
                    ),
                    onPressed: () {
                      setState(() {
    ...
    for (int i = 0; i < colorList.length; i++) {
      ...
      widgetList.add(GestureDetector(
          ...
          child: Container(
            ...
            decoration:
                BoxDecoration(color: colorList[i], shape: BoxShape.circle),
            child: Icon(
              Icons.check,
              color: ThemeData.estimateBrightnessForColor(colorList[i]) ==
                      Brightness.dark
                  ? Colors.white.withOpacity(visible[i])
                  : Colors.black.withOpacity(visible[i]),
            ),
          )));
    }
    return widgetList;
  ...
  List<Widget> colorsCircles() {
    ...
    colorList.forEach((color) {
      list.add(CircleAvatar(
        ...
        radius: 20,
        child: currentColorIndex == i
            ? Icon(Icons.check, color: Colors.black.withOpacity(1))
...
class FormulaItemContainer extends StatefulWidget {
  ...
  @override
  _FormulaItemContainer createState() =>
      _FormulaItemContainer(item: item, isCustom: isCustom);
}
                ...
                RaisedButton(
                    ...
                    onPressed: () {
                      ...
                      item.formula.variables.forEach((variable) {
                        double value;
                        Unit currentUnit =
                            Units.getUnitFromString(item.units[i]);
                        ...
                        if (item.units[i] != currentUnit.main &&
                            value != 0 &&
                            variable != item.formula.result) {
                          value = currentUnit.getValueOf(item.units[i], value,
                              resultUnit: currentUnit.main);
                        }
                        values[variable] = value;
                        i++;
                      });

                      String resultUnitString = item.units[
                          item.formula.variables.indexOf(item.formula.result)];
                      Unit resultUnit =
                          Units.getUnitFromString(resultUnitString);
                      double result = double.parse(item.formula.calc(values));
                      setState(() {
                        item.result = resultUnitString == resultUnit.main
                            ? result.toString()
                            : resultUnit
                                .getValueOf(resultUnit.main, result,
                                    resultUnit: resultUnitString)
                                .toString();
                      });
        ...
        child: Row(
          ...
          children: [
            Expanded(
                ...
                child: Center(
                    child: CaTeX(
                        Formula2.toCaTeXstatic([item.formula.variables[i]])))),
            Padding(
              padding: const EdgeInsets.only(left: 2, right: 2),
              child: Text('='),
            ),
            Expanded(flex: 125, child: Text(item.meanings[i])),
            Expanded(
              flex: 80,
              child: Container(
            ...
            Expanded(
              ...
              child: DropdownButton(
                  items:
                      Units.getUnitFromString(item.units[i]).getDropdownList(),
                  underline: Container(),
                  isDense: true,
                  isExpanded: true,
...
class FormulaExpansionListOld extends StatefulWidget {
  ...
  @override
  _FormulaExpansionListOldState createState() =>
      _FormulaExpansionListOldState(data: data, isCustom: isCustom);
}
...
class _FormulaExpansionListOldState extends State<FormulaExpansionListOld> {
      ...
      allWidgets.add(
        Container(
            ...
            child: Column(children: [
              ...
              Container(
                margin: EdgeInsets.only(bottom: 8),
                child: Center(child: getTitleWidget(item)),
              ),
              Container(
                //margin: EdgeInsets.only(bottom: 4),
                child: Column(children: getLegend(item)),
              ),
              Container(
                          ...
                          onPressed: () {
                            ...
                            item.formula.variables.forEach((variable) {
                              ...
                              if (item.controllerList[i].text == '') {
                                value = 0.0;
                                logcat(
                                    'value for variable(i: $i): $variable = 0');
                              } else {
                                value =
                                    double.parse(item.controllerList[i].text);
                                logcat(
                                    'value for variable: $variable = $value');
                              }
                              values[variable] = value;
                              i++;
                            });
                            ...
                            int controllerIndex = 0;
                            for (int i = 0;
                                i < item.formula.variables.length;
                                i++) {
                              if (item.formula.variables[i] ==
                                  item.formula.result) {
                                controllerIndex = i;
                                break;
                              }
                            ...
                            item.controllerList[controllerIndex].text =
                                item.result;
                          }),
                    ),
                  ],
  ...
  List<Widget> getLegend(Item item) {
    List<Widget> widgetList = [];
    item.meanings ??=
        List.generate(item.formula.variables.length, (index) => 'null');
    ...
    for (int i = 0; i < item.formula.variables.length; i++) {
      if (item.meanings[i] != '') {
         ...
         */
        if (item.units != null &&
            item.units.length == item.formula.variables.length) {
                  ...
                  child: Row(
                    children: [
                      CaTeX(
                          Formula2.toCaTeXstatic([item.formula.variables[i]])),
                      Expanded(
                          child: item.meanings[i] == 'null'
                              ? Text(' =')
                              : Text(' = ' + item.meanings[i])),
                    ],
                  ),
                ),
                Expanded(
