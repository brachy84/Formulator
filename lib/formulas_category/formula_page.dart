...
void initController(List<Item> data) {
  if(data.first.controllerList == null || data.first.controllerList.length < 1) {
    data.forEach((item) {
...
void disposeController(List<Item> data) {
  if(data.first.controllerList != null || data.first.controllerList.length > 0) {
    data.forEach((item) {
...
List<Widget> getSubCategoryCard(List<FormulaSubCategoryBase> subCategories, BuildContext context) {
  List<Widget> widgets = [];
...
class FormulaSubCategoryBase extends StatefulWidget {
  ...
  FormulaSubCategoryBase({@required this.name, this.color, this.icon, @required this.data}) {
    color ??= Colors.grey;
    icon ??= FaIcon(FontAwesomeIcons.plus);
  }

  @override
  _FormulaSubCategoryBaseState createState() => _FormulaSubCategoryBaseState(name, color, icon, data);
}

class _FormulaSubCategoryBaseState extends State<FormulaSubCategoryBase> with TickerProviderStateMixin {
      ...
      widgets.add(OpenContainer(
        ...
        openBuilder: (context, action) {
          return Scaffold(
              ...
              body: FormulaItemContainer(item: item, isCustom: false,)
          );
        },
      ));
      i++;
