...
class Calculate {
  ...
  bool hasSimpleMultiplicator(List<String> formula) {
    ...
    while(copy.contains('*')) {
      ...
      if(Utils.isNumeric(copy[index-1]) && Utils.isNumeric(copy[index+1])) {
        return true;
      } else {
        copy[index] = '#';
      }
  ...
  Map<List<String>, int> calcBase(List<String> formula) {
    ...
    if (formula.length >= 3 && formula.length % 2 == 1 && !Formula.isOperator(formula[0])&& !Formula.isOperator(formula[2])) {
      logcat('  calc');
      while(formula.length > 1) {
        ...
        switch(formula[1]) {
          case '+' :
            formula.replaceRange(0, 3, [(double.parse(formula[0]) + double.parse(formula[2])).toString()]); break;
          case '-' :
            formula.replaceRange(0, 3, [(double.parse(formula[0]) - double.parse(formula[2])).toString()]); break;
          case '*' :
            formula.replaceRange(0, 3, [(double.parse(formula[0]) * double.parse(formula[2])).toString()]); break;
          default : throw FormatException('Second position was not *, + or -.');
        //case '' :
        }
      }
    } else {
      ...
      for(int i = 0; i < formula.length; i++){
        if(formula[i].contains('[')) {
          if(formula[i+2] == ']') {
            ...
            switch(operation) {
              case 'sin' : formula[i+1] = sin(Utils.toRad(num)).toString(); break;
              case 'cos' : formula[i+1] = cos(Utils.toRad(num)).toString(); break;
              case 'tan' : formula[i+1] = tan(Utils.toRad(num)).toString(); break;
              case 'asin' : formula[i+1] = Utils.toDeg(asin(num)).toString(); break;
              case 'acos' : formula[i+1] = Utils.toDeg(acos(num)).toString(); break;
              case 'atan' : formula[i+1] = Utils.toDeg(atan(num)).toString(); break;
              default :
            ...
            List<String> braceContent = formula.getRange(i+1, calcFormula.findClosingIndex(i, type: '[]', list: formula)).toList();
            ...
            formula.replaceRange(i+1, calcFormula.findClosingIndex(i, type: '[]', list: formula), braceContent);
  ...
  List<String> solve(List<String> formula, {bool isNested = false}) {
    ...
    logcat('  solveBrace with formula (calc step4): $formula | isNested: $isNested');
    /// //////////////////////////////////////////
    while(formula.contains('(')) {
      ...
      while(braceContent.contains('(')) {
        for(int i = 0; i < formula.length; i++) {
          if(formula[i] == '(') {
            braceContent = formula.getRange(i+1, calcFormula.findClosingIndex(i, list: formula, type: '()')).toList();
            break;
          }
        }
      }

      Calculate calculator = Calculate(calcFormula: Formula('#AP = ' + braceContent.join(' ')));
      ...
      if(formula.contains('(')) {
        ...
        for(int i = 0; i < formula.length; i++) {
          if(formula[i] == '(') {
            logcat('    found (');
            formula.replaceRange(i, calcFormula.findClosingIndex(i, list: formula, type: '()')+1, braceContent);
            break;
          }
        }
      } else {
  ...
  List<String> solveMultiplications(List<String> formula) {
    ...
    while(formula.contains('<')) {
      if(formula.contains('<')) {
        braceContent = formula.getRange(formula.indexOf('<')+1, calcFormula.findClosingIndex(formula.indexOf('<'), type: '<>', list: formula)).toList();
        /*
        while(braceContent.contains('<')) {
          logcat('  has <');
          braceContent = solveMultiplications(braceContent);
      ...
      if(formula.contains('<')) {
        for(int i = 0; i < formula.length; i++) {
          if(formula[i] == '<') {
            logcat('    found < at $i');
            logcat('    formula before replacing: $formula');
            formula.replaceRange(i, calcFormula.findClosingIndex(i, type: '<>', list: formula)+1, braceContent);
 ...
 */
  ...
  List<String> solveCurlyBrace(List<String> formula) {
    ...
    if(formula.contains(':{') || formula.contains('√{')) {
      for(int i = 0; i < formula.length; i++) {
        if(formula[i].contains('{')) {
          if(formula[i] == ':{') {
            braceContent = formula.getRange(i+1, calcFormula.findClosingIndex(i, findMidOperator: true, list: formula)).toList();
          } else if(formula[i] == '}/{' || formula[i] == '√{') {
            braceContent = formula.getRange(i+1, calcFormula.findClosingIndex(i, list: formula)).toList();
          }
          if(braceContent.length >= 3 && !braceContent.contains(':{') && !braceContent.contains('√{') && !braceContent.contains('}/{')) {
          ...
          if(formula[i] == ':{') {
            formula.replaceRange(i+1, calcFormula.findClosingIndex(i, findMidOperator: true, list: formula), braceContent);
          } else if(formula[i] == '}/{' || formula[i] == '√{') {
            formula.replaceRange(i+1, calcFormula.findClosingIndex(i, list: formula), braceContent);
          }
  ...
  ///Solves {}. Now all {} should be gone.
  List<String> dissolveCurlyBrace(List<String> formula, {bool isNested = false}) {
    ...
    while(formula.contains(':{') || formula.contains('√{')) {
      ...
      if(isDivide) {
        ...
        logcat('  double top: ${braceContent.getRange(0, braceContent.indexOf('}/{')).join(' ')}');
        logcat('  double bottom: ${braceContent.getRange(braceContent.indexOf('}/{')+1, braceContent.length).join(' ')}');
        braceContent = [(double.parse(braceContent.getRange(0, braceContent.indexOf('}/{')).join(' ')) / double.parse(braceContent.getRange(braceContent.indexOf('}/{')+1, braceContent.length).join(' '))).toString()];
      } else if(!isDivide) {
        logcat(' now rooting / braceContent: $braceContent');
        braceContent = [sqrt(double.parse(braceContent.getRange(0, braceContent.length).join(' '))).toString()];
      }
      ...
      if(formula.contains(':{') || formula.contains('√{')) {
        ...
        for(int i = 0; i < formula.length; i++) {
          if(formula[i] == ':{' || formula[i] == '√{') {
            logcat('    found :{ / √{');
            formula.replaceRange(i, calcFormula.findClosingIndex(i, list: formula)+1, braceContent);
            break;
          }
        }
      } else {
