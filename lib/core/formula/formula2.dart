...
class Formula {
  ...
  Formula(this.raw) {
    ...
    _sidedValues.forEach((side) {
      side.forEach((value) {
        bool isDupe = false;
        if(!isOperator(value) && value != '=' && !variables.contains(value) && !Utils.isNumeric(value) && !isConstant(value)) {
          if(value.contains('^')) {
            String WOpow = value.replaceRange(value.indexOf('^'), value.length, '');
            value = WOpow;
          ...
          if(!isDupe) {
            if(value.length > 1 && !value.contains('_') && !isGreekLetter(value)) {
  ...
  void parseRaw({List<String> sides, bool determineSides = false, bool switchTrigonometric = true}) {
          ...
          default:
            {
              if (isTypeOf(current, lastChar)) {
                ...
                if (((isFriendlyOperator(current) && isFriendlyOperator(lastChar)) || !(isOperator(current) && isOperator(lastChar))) && lastChar != current) {
    ...
    applyToFormula(determineSides: determineSides, switchTrigonometric: switchTrigonometric);
    //print('parsing: $_parsingValues');
    //applyToFormula();
  }

  void checkBrackets() {
    if(sidedValues[0].first.contains('[') && sidedValues[0][sidedValues[0].length-2] == ']') {
      switch(sidedValues[0].first) {
        case 'sin[' :
          sidedValues[1].insert(0, 'asin[');
          sidedValues[1].add(']'); break;
        case 'cos[' :
  ...
  void applyToFormula({bool determineSides = false, bool switchTrigonometric = true}) {
  ...
  void changeTo(String result) {
    if(result != originalResult) {
      if (!_values.contains(result) || isOperator(result)) {
        ...
        if(!foundExpResult)
          throw FormatException('Formula can not be changed to an operator or an not existent variable.');
      }
    }
      ...
      if (isInRoot(result)) {
        if (_sidedValues[1].first == '√{' && _sidedValues[1].last == '}') {
          ...
          if(hasPostVars) {
            ...
            clearLine(result, start: findClosingIndex(_sidedValues[1].indexOf('√{')) + 1, end: _sidedValues[1].length - 1);
          }
          if(hasPreVars)
            clearLine(result, start: 0, end: _sidedValues[1].indexOf('√{')-1);

          square();
        }
      } else if(_sidedValues[1].contains('√{')) {
        print('sidedValues: $_sidedValues');
        switchSideOf(getRangeAsString(_sidedValues[1].indexOf('√{'), findClosingIndex(_sidedValues[1].indexOf('√{'))));
      }
      ...
      while(_sidedValues[1].contains(':{')) {
        ...
        if(firstIndex != lastIndex) {
          ...
          if(isInDivider(result, index: firstIndex)) {
            ...
            //clear lastIndex
            String top = _sidedValues[1].getRange(_sidedValues[1].lastIndexOf(':{') + 1, findClosingIndex(_sidedValues[1].lastIndexOf(':{'), findMidOperator: true)).join(' ');
            multiply(List<String>.from(_sidedValues[1].getRange(_sidedValues[1].lastIndexOf('}/{') + 1, findClosingIndex(_sidedValues[1].lastIndexOf('}/{')))));
            switchSideOf(top);
          } else {
            logcat('Result is in last Divider');
            //clear first index
            String top = _sidedValues[1].getRange(_sidedValues[1].indexOf(':{') + 1, findClosingIndex(_sidedValues[1].indexOf(':{'), findMidOperator: true)).join(' ');
            multiply(List<String>.from(_sidedValues[1].getRange(_sidedValues[1].indexOf('}/{') + 1, findClosingIndex(_sidedValues[1].indexOf('}/{')))));
            switchSideOf(top);
          }
          ...
          if (_sidedValues[1].first == ':{' && _sidedValues[1].last == '}') {
            if (isNumerator(result)) {
              ...
              divide(List<String>.from(_sidedValues[1].getRange(_sidedValues[1].indexOf(':{') + 1, _sidedValues[1].indexOf('}/{'))), areNumerators: true);
            }
          } else {
          ...
          String divider = _sidedValues[1].getRange(_sidedValues[1].indexOf(':{'), findClosingIndex(_sidedValues[1].indexOf(':{'))+1).join(' ');

          if(_sidedValues[0].contains(':{') || _getNextOperatorOfDivider(divider) == '*') {
            String top = _sidedValues[1].getRange(_sidedValues[1].indexOf(':{') + 1, findClosingIndex(_sidedValues[1].indexOf(':{'), findMidOperator: true)).join(' ');
            multiply(List<String>.from(_sidedValues[1].getRange(_sidedValues[1].indexOf('}/{') + 1, findClosingIndex(_sidedValues[1].indexOf('}/{')))));
            switchSideOf(top);
          } else {
  ...
  String getRangeAsString(int start, int end, {int side = 1}) {
    ...
    return List<String>.from(_sidedValues[side].getRange(start, end+1)).join(' ');
  }
      ...
      cdotList.forEach((index) {
        if(lastIndex != 0) {
          if(index - lastIndex == 2) {
            if(!(isOperator(_sidedValues[side][index-2]) && isOperator(_sidedValues[side][index-1])) && (_sidedValues[side][index-2] != result && _sidedValues[side][index-1] != result)) {
              multiplierLines.last += ' ' + _sidedValues[side][index-2] + ' ' + _sidedValues[side][index-1];
            }
          } else {
            if(!isOperator(_sidedValues[side][index-1]) && _sidedValues[side][index-1] != result) {
  ...
  void switchSideOf(String value) {
    ...
    if (valueIndex == 0 || _values[valueIndex - 1] == '=') {
      valueIndex = start > 0 ? value.split(' ').length-1 : _values.indexOf(value.split(' ')[0]);
  ...
  String calc(Map<String, double> values) {
    ...
    logcat(' values before step 1: $calcValues');
      for(int i = 0; i < calcValues.length; i++) {
        ...
        if(calcValues[i].length > 1 && !isOperator(calcValues[i]) && !Utils.isNumeric(calcValues[i])) {
          int numLength = 0;
    ...
    for(int i = 0; i < calcValues.length; i++) {
      ...
      if(!isOperator(calcValues[i]) && calcValues[i] != '=' && !Utils.isNumeric(calcValues[i])) {
        logcat('  replacable');
        if (calcValues[i] == 'pi')
          ...
          if (parseValue.contains('^')) {
            ...
            parseValue = parseValue.replaceRange(parseValue.indexOf('^'), parseValue.length, '');
  ...
  List<String> parseForCalc(List<String> formula) {
    if(formula.contains('*')) {
      for(int i = 0; i < formula.length; i++) {
        if(formula[i] == '*') {
          if(Utils.isNumeric(formula[i+1]) && Utils.isNumeric(formula[i-1])) {
            formula.insert(i-1, '<');
            i++;
            formula.insert(i+2, '>');
            i++;
  ...
  void divide(List<String> values, {String operator = ' ', bool areNumerators = false}) {
      ...
      if (containsAll) {
        ...
        if (areNumerators) {
          ...
          if(lastAction == '*') {
            ...
            parsedSide = getRangeAsString(0, findClosingIndex(_sidedValues[i].indexOf(':{'),side: i)-1, side: i) + ' * ' + values.join(operator) + getRangeAsString(findClosingIndex(_sidedValues[i].indexOf(':{'), side: i),  _sidedValues[i].length-1, side: i);
    ...
    _parsedSides.forEach((parsedSide) {
      if (parsedSide.contains('√{') && parsedSide.contains('}')) {
        ...
        for (int j = openingBraceIndex + 1; j < _sidedValues[i].length; j++) {
          ...
          if (_sidedValues[i][j].contains('{')) count++;
          if (count == 0) closingBraceIndex = j - 1;
          else print('SQUARE: closingIndex not found');
        }
        if (power > 2) {
        ...
        print('closingIndex: $closingBraceIndex | openingIndex $openingBraceIndex');
  ...
  void root({int power = 2, bool rootResult = false}) {
    ...
    if(rootResult) {
      if(_sidedValues[0].first.contains('^')) {
        ...
        _sidedValues[0].first = val.replaceRange(val.indexOf('^'), val.length, '');
      }
  ...
  void switchSideOfBrackets({int fromSide = 1}) {
    ...
    if(sidedValues[fromSide].first.contains('[') && sidedValues[fromSide][sidedValues[fromSide].length-2] == ']') {
      switch(sidedValues[fromSide].first) {
        case 'sin[' :
          _sidedValues[toSide].insert(0, 'asin[');
          sidedValues[toSide].add(']'); break;
        case 'cos[' :
  ...
  bool isInDivider(String value, {int index, int side = 1}) {
    ...
    while (true) {
      if (_sidedValues[side][i] == value) {
        ...
        if (_sidedValues[side][i].contains('{') && !_sidedValues[side][i].contains('}/{')) count++;
        if (_sidedValues[side][i].contains('}') && !_sidedValues[side][i].contains('}/{')) count--;
        logcat('   currently at i $i ((${_sidedValues[side][i]})) with count $count');
        if (count == 0) return false;
        i++;
      }
    }
  ...
  bool isNumerator(String value) {
    ...
    _sidedValues.forEach((side) {
      if(side.contains(value)) {
        ...
        int count = 1;
        while (i >= 0) {
          if (side[i] == value){ isTrue = true; break;}
          if (side[i].contains('{')) count--;
          if (side[i].contains('}')) count++;
          if (count == 0){ isTrue = false; break;}
          i--;
        }
      }
    });
  ...
  static bool isCalcOperator(String variable) => calcOperators.contains(variable);
  static bool isFriendlyOperator(String variable) => friendlyOperators.contains(variable);
  ...
  static final List<String> greekLetters = [
    'alpha', 'beta', 'gamma', 'delta', 'epsilon', 'zeta', 'eta', 'theta', 'iota', 'kappa', 'lambda', 'mu', 'nu', 'ksi', 'omicron', 'pi', 'rho', 'sigma', 'tau', 'ypsilanti', 'phi', 'chi', 'omega'
  ];

  static final List<String> constants = [
    'pi'
  ];
